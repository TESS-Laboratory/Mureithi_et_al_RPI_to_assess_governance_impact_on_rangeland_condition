...
```{r}
# Increasing IMAGE size above 
tmap_mode("plot")
tmap_options(component.autoscale = FALSE)  # Prevent rescaling warnings



slope_range <- range(c(
  values(Amboseli.Slope.Raster),
  values(TsavoEast.Slope.Raster),
  values(TsavoWest.Slope.Raster),
  values(Tsavo.Ranches.Slope.Raster),
  values(Tsavo.SettledAreas.Slope.Raster),
  values(Tsavo.AllZones.Slope.Raster),
  values(AET.Conservancies.Slope.Raster),
  values(AET.Buffers.Slope.Raster),
  values(AET.AllZones.Slope.Raster)
), na.rm = TRUE)

# Arrange maps in 1 row × 9 columns with bold titles
Combined.Slope.Maps <- tmap_arrange(
  tm_shape(Amboseli.Slope.Raster) +
    tm_raster(
      col.scale = tm_scale_continuous(values = "pu_or", midpoint = 0, limits = slope_range),
  ...
  tm_shape(AET.AllZones.Slope.Raster) +
    tm_raster(
      col.scale = tm_scale_continuous(values = "pu_or", midpoint = 0, limits = slope_range),
      col.legend = tm_legend(title = "Slope", reverse = TRUE)  # only this one shows
    ) +
    tm_title("AET All Zones") +
    tm_layout(title.fontface = "bold"),
...
#tmap_save(
#  tm = Combined.Slope.Maps,
#  filename = "Theil_Sen_Slope_Maps_BoldTitles.png",
#  width = 8.27,   # portrait A4 width in inches
#  height = 11.7,  # portrait A4 height in inches
#  dpi = 300
#)
```
...
RPI.Ridgeline.Simulation.Plot <- ggplot(RPI.All.Simulation, aes(x = RPI, y = Area, fill = Area)) +
  ...
  # Replace viridis with manual color scale 
