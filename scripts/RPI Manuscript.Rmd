...
RPI.All.Simulation <- RPI.All.Simulation %>%
  mutate(Area = recode(Area, !!!area_labels.v1))
...
RPI.Ridgeline.Simulation.Plot <- ggplot(RPI.All.Simulation,
  ...
  geom_density_ridges(
    ...
    scale = 1.2,
    color = "black",
    linewidth = 0.3   # ✅ correct argument (not size)
  ) +
...
# Recoding with my preferred labels
All.Pixel.Slopes <- All.Pixel.Slopes %>%
  mutate(Area = recode(Area, !!!area_labels1))

# Ridgeline plot with median line to match RPI plot style
TheilSen.Ridgeline.Plot <- ggplot(All.Pixel.Slopes, aes(x = Slope, y = Area, fill = Area)) +
  ...
  # Use the new colors
  scale_fill_manual(values = area_colors_simulation, name = "Study Area") +
  
  # Use new labels for y-axis
  scale_y_discrete(labels = area_labels1) +
...
RPI.All.Simulation.No.AllZones <- RPI.All.Simulation %>%
  dplyr::filter(!Area %in% areas_to_remove)

# Reorder factor levels
Mean.RPI.Order.No.AllZones <- RPI.All.Simulation.No.AllZones %>%
  group_by(Area) %>%
  summarise(MeanRPI_Area = mean(RPI, na.rm = TRUE)) %>%
  arrange(MeanRPI_Area) %>%
  pull(Area)

RPI.All.Simulation.No.AllZones$Area <-
  factor(RPI.All.Simulation.No.AllZones$Area,
         levels = Mean.RPI.Order.No.AllZones)
...
# Saving the plot
ggsave(
  filename = "RPI_Ridgeline_No_AllZones.png",
  plot = RPI.Ridgeline.Simulation.No.AllZones,
  width = 8,
  height = 6,
  units = "in",
  dpi = 300
...
# Read images
Mean.RPI.Static.Map.Image.1 <- image_read("Mean_RPI_Static_Map.png")

Study.Areas.Map.Composite.Image.1 <- image_read("Study_Areas_Map_Composite_Coloured.png")

RPI.Ridgeline.Map.Image.1 <- image_read("RPI_Ridgeline_No_AllZones.png")

# Convert to raster grobs
grob.Mean.RPI.Static.Map.Image.1 <- rasterGrob(as.raster(Mean.RPI.Static.Map.Image.1))

grob.Study.Areas.Map.Composite.Image.1 <- rasterGrob(as.raster(Study.Areas.Map.Composite.Image.1))

grob.RPI.Ridgeline.Map.Image.1 <- rasterGrob(as.raster(RPI.Ridgeline.Map.Image.1))

# Wrap each raster in ggplot
ppA.Mean.RPI.Static.Map.Image.1 <- ggplot() + annotation_custom(grob.Mean.RPI.Static.Map.Image.1, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + theme_void()

ppB.Study.Areas.Map.Composite.Image.1 <- ggplot() + annotation_custom(grob.Study.Areas.Map.Composite.Image.1, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + theme_void()

ppC.RPI.Ridgeline.Map.Image.1 <- ggplot() + annotation_custom(grob.RPI.Ridgeline.Map.Image.1, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + theme_void()

# Arrange with patchwork and labels
Mean.RPI.Static.Study.Areas.Composite.RPI.Ridgeline.Map.Patchwork <- ppA.Mean.RPI.Static.Map.Image.1 / (ppB.Study.Areas.Map.Composite.Image.1 + ppC.RPI.Ridgeline.Map.Image.1) +
  plot_annotation(tag_levels = 'A') &
  theme(
...
# Removing the white spaces in the image above

# OPTIONAL BELOW: Trim whitespace from source PNGs~ (Run once if your original PNGs contain margins)
image_read("Mean_RPI_Static_Map.png") |>
  image_trim() |>
  image_write("Mean_RPI_Static_Map_trim.png")

image_read("Study_Areas_Map_Composite_Coloured.png") |>
  image_trim() |>
  image_write("Study_Areas_Map_Composite_Coloured_trim.png")

image_read("RPI_Ridgeline_No_AllZones.png") |>
  image_trim() |>
  image_write("RPI_Ridgeline_No_AllZones_trim.png")

# Wrap images properly (NO rasterGrob needed)
ppA <- ggdraw() + 
  draw_image("Mean_RPI_Static_Map_trim.png")

ppB <- ggdraw() + 
  draw_image("Study_Areas_Map_Composite_Coloured_trim.png")

ppC <- ggdraw() + 
  draw_image("RPI_Ridgeline_No_AllZones_trim.png")

# Patchwork Layout
# Top: Mean RPI
# Bottom: Study Areas | Ridgeline

Mean.RPI.Static.Study.Areas.Composite.RPI.Ridgeline.Map.Patchwork.2 <-
  ppA /
  (ppB + ppC) +
  plot_layout(
    heights = c(1.25, 1)  # Adjust if top needs more/less space
  ) +
  plot_annotation(tag_levels = "A") &
  theme(
...
# Save A4 Portrait (Publication Ready)
ggsave(
  filename = "Mean.RPI.Static.Study.Areas.Composite.RPI.Ridgeline.Map.Patchwork.2.png",
  plot = Mean.RPI.Static.Study.Areas.Composite.RPI.Ridgeline.Map.Patchwork.2,
...
```{r}
# VERSION 1 for saving the 3 images (Slope_Map_Composite | Study_Areas_Map_Composite_Coloured | Ridgeline_Plot_No_AllZones)
...
# Arrange with patchwork and labels
Slope.Map.Composite.Study.Areas.Map.Composite.Ridgeline.Plot.Patchwork <- ppA.Slope.Map.Composite.Image.1 / (ppB.Study.Areas.Map.Composite.Image.1 + ppC.Ridgeline.Plot.No.AllZones.Image.1) +
  plot_annotation(tag_levels = 'A') &
  theme(
    plot.tag = element_text(size = 12, face = 'bold'),
    panel.spacing = unit(0.2, "cm"),
...
```{r}
# VERSION 2 for saving the 3 images (Slope_Map_Composite | Study_Areas_Map_Composite_Coloured | Ridgeline_Plot_No_AllZones)


# Trimming whitespace (run once only when needed)


image_read("Slope_Map_Composite.png") |>
  image_trim() |>
  image_write("Slope_Map_Composite_trim.png")

image_read("Study_Areas_Map_Composite_Coloured.png") |>
  image_trim() |>
  image_write("Study_Areas_Map_Composite_Coloured_trim.png")

image_read("Ridgeline_Plot_No_AllZones.png") |>
  image_trim() |>
  image_write("Ridgeline_Plot_No_AllZones_trim.png")

# Wrap images using ggdraw (NO rasterGrob)
ppA <- ggdraw() + 
  draw_image("Slope_Map_Composite_trim.png")

ppB <- ggdraw() + 
  draw_image("Study_Areas_Map_Composite_Coloured_trim.png")

ppC <- ggdraw() + 
  draw_image("Ridgeline_Plot_No_AllZones_trim.png")

# Patchwork Layout
# Top: Slope Map
# Bottom: Study Areas | Ridgeline
Slope.Map.Composite.Study.Areas.Map.Composite.Ridgeline.Plot.Patchwork.2 <-
  ppA /
  (ppB + ppC) +
  plot_layout(
    heights = c(1.25, 1)   # adjust if slope map needs more space
  ) +
  plot_annotation(tag_levels = "A") &
  theme(
    plot.tag = element_text(size = 18, face = "bold"),
...
# Save A4 Portrait (Publication Ready)
ggsave(
  filename = "Slope.Map.Composite.Study.Areas.Map.Composite.Ridgeline.Plot.Patchwork.2.png",
  plot = Slope.Map.Composite.Study.Areas.Map.Composite.Ridgeline.Plot.Patchwork.2,
  width = 8.27,
  height = 11.69,
  units = "in",
  dpi = 600,
