...
pacman::p_load(sf, terra, tidyverse, tidyterra, colorspace, ggrepel, patchwork, tmap, trend, slider, forcats, viridis, viridisLite, scales, hexbin, ggh4x, scico, ggridges, truncnorm, zoo, cowplot, ggplotify, ggspatial, png, magick, ggtext, grid, shinyjs, kableExtra, colorblindcheck, plotly, dplyr, ggplot2, rstatix, mgcv, kableExtra, flextable)
...
# kableExtra -> for enhancing tables created with knitr::kable()
# flextable → for creating and customising flexible, publication‑quality tables
...
```{r}
# Combined Mean RPI Trends with Theil–Sen Slopes
...
Combined.Trends <- Annual.RPI.Visual.Trends %>%
  left_join(
    All.Trends.Summary %>%
      dplyr::select(Area, MeanSlope),
    by = "Area"
  )

Combined.Trends.Plot <- ggplot(
  Combined.Trends,
  aes(x = Year, y = MeanRPI)
) +
  ...
  # Theil–Sen Trend Line (Colour-Coded Solid)
  geom_smooth(
    aes(color = Area),
    method = "lm",
    formula = y ~ x,
...
```{r}
# Modifying the MAE, RSME AND R^2

# names(Temporal.Performance.Kenya) <- c("mae","rmse","rsq")

# 1. MAE
Map.MAE <- tm_shape(Temporal.Performance.Kenya[["mae"]]) +
  tm_raster(
  col.scale = tm_scale_intervals(values = "scico.roma", style = "quantile", n = 7, midpoint = NA),
  col_alpha = 0.8,
  col.legend = tm_legend(title = "MAE")
) +
  tm_shape(Kenya.Admin.Boundary) +
  tm_borders(col = "black", lwd = 0.5) +
  tm_title("Mean Absolute Error") +
  tm_layout(legend.outside = TRUE) +
  tm_view(basemap.server = c(
    "Esri.WorldImagery",
    "OpenStreetMap",
...
# 2. RMSE
Map.RMSE <- tm_shape(Temporal.Performance.Kenya[["rmse"]]) +
  tm_raster(
    col.scale = tm_scale_intervals(values = "scico.vik", style = "quantile", n = 7, midpoint = NA),
    col_alpha = 0.8,
    col.legend = tm_legend(title = "RMSE")
  ) +
  tm_shape(Kenya.Admin.Boundary) +
  tm_borders(col = "black", lwd = 0.5) +
  tm_title("Root Mean Squared Error") +
  tm_layout(legend.outside = TRUE) +
  tm_view(basemap.server = c(
    "Esri.WorldImagery",
    "OpenStreetMap",
...
# 3. R²
Map.Rsqd <- tm_shape(Temporal.Performance.Kenya[["rsq"]]) +
  tm_raster(
    col.scale = tm_scale_intervals(values = "scico.bam", style = "quantile", n = 7, midpoint = NA),
    col_alpha = 0.8,
    col.legend = tm_legend(title = expression(R^2))
  ) +
  tm_shape(Kenya.Admin.Boundary) +
  tm_borders(col = "black", lwd = 0.5) +
  tm_title(expression(R^2)) +
  tm_layout(legend.outside = TRUE) +
  tm_view(basemap.server = c(
    "Esri.WorldImagery",
    "OpenStreetMap",
...
# Assign the arranged map to an object
Combined.Temporal.Performance.TMap <- tmap_arrange(Map.MAE, Map.RMSE, Map.Rsqd, ncol = 3)
Combined.Temporal.Performance.TMap
```

```{r}
# Saving the above plot
#tmap_save(
#  tm = Combined.Temporal.Performance.TMap,
#  filename = "Combined_Temporal_Performance_TMap.png",
#  width = 8.27,
#  height = 11.69,
#  units = "in",
#  dpi = 600
...
# Turning Area into an ordered factor ranked from lowest to highest median slope
# Compute median slope per area and reorder Area as a factor
area_order <- All.Pixel.Slopes %>%
  group_by(Area) %>%
  summarise(median_slope = median(Slope, na.rm = TRUE)) %>%
  arrange(median_slope)

All.Pixel.Slopes <- All.Pixel.Slopes %>%
  mutate(Area = factor(Area, levels = area_order$Area))
...
```{r}
# Creating a "Specific.Study.Areas.Map.Coloured" within Kenya Administrative Boundary

Main.Study.Area.Map <- 
  # Regional admin boundary as background context
  tm_shape(Kenya.Regional.Admin.Boundary) +
  tm_polygons(
    fill = "grey90",
    col = "grey40",
...
# Saving the above plot
#tmap_save(
#  tm = Main.Study.Area.Map,
#  filename = "Main.Study.Area.Map.Plot.png",
#  width = 8.27,
#  height = 11.69,
#  units = "in",
#  dpi = 600
...
tmap_mode("plot")
RPI.Filtered1.Unified.ContinuousLegend.Map <- 
...
# Saving the above plot
#tmap_save(
#  tm = RPI.Filtered1.Unified.ContinuousLegend.Map,
#  filename = "RPI_Filtered1_Unified_ContinuousLegend_Map.png",
#  width = 8.27,
#  height = 11.69,
#  units = "in",
#  dpi = 600
...
tmap_mode("plot")
# Update the Map
RPI.Filtered1.Unified.BreakLegend.Map <- 
  tm_shape(Combined.RPI.Filtered.1.Raster) +
  tm_raster(
...
# Saving the above plot
#tmap_save(
#  tm = RPI.Filtered1.Unified.BreakLegend.Map,
...
TheilSen.Slope.Unified.BreakLegend.Map <- 
...
# Saving the above plot
#tmap_save(
#  tm = TheilSen.Slope.Unified.BreakLegend.Map,
...
```{r}
# Continuous Theil–Sen Slope Map (No Interval Breaks) to differentiate with above map

# Optional: Define display limits for the continuous slope scale
# Helps prevent extreme outliers from dominating the colour stretch
Slope.Limits <- c(-0.05, 0.05)

# Continuous Theil–Sen Slope Map
TheilSen.Slope.Unified.ContinuousLegend.Map <- 
...
# Saving the above plot
#tmap_save(
#  tm = TheilSen.Slope.Unified.ContinuousLegend.Map,
#  filename = "TheilSen_Slope_Unified.ContinuousLegend.Map.png",
#  width = 8.27,
#  height = 11.69,
#  units = "in",
#  dpi = 600
...
```{r}
# Creating a table indicating RPI values above 1 within the entire study area

# Combine all actual pixel-level RPI data with area labels
RPI.Actual.All <- bind_rows(
  Amboseli.Values.RPI.Rsquare.Filtered.1 %>% mutate(Area = "Amboseli NP"),
  AET.Conservancies.Values.RPI.Rsquare.Filtered.1 %>% mutate(Area = "Amboseli Conservancies"),
  AET.Buffers.Values.RPI.Rsquare.Filtered.1 %>% mutate(Area = "Amboseli Community/Group Ranches"),
  TsavoEast.Values.RPI.Rsquare.Filtered.1 %>% mutate(Area = "Tsavo East NP"),
  TsavoWest.Values.RPI.Rsquare.Filtered.1 %>% mutate(Area = "Tsavo West NP"),
  Tsavo.Ranches.Values.RPI.Rsquare.Filtered.1 %>% mutate(Area = "Tsavo Conservancies/Ranches"),
  Tsavo.SettledAreas.Values.RPI.Rsquare.Filtered.1 %>% mutate(Area = "Tsavo Settled Areas")
)

# Compute proportion with RPI > 1 per area
RPI.Above1.Table <- RPI.Actual.All %>%
  group_by(Area) %>%
  summarise(
    Total_Pixels = n(),
    Pixels_Above_1 = sum(RPI > 1, na.rm = TRUE),
    Proportion = round(Pixels_Above_1 / Total_Pixels, 4),
...
```{r}
# Making the above table publication worthy

#library(kableExtra)

RPI.Above1.Table %>%
  select(Area, Total_Pixels, Pixels_Above_1, Percentage) %>%
  rename(
    `Study Area` = Area,
    `Total Pixels` = Total_Pixels,
    `Pixels with RPI > 1` = Pixels_Above_1,
    `% with RPI > 1` = Percentage
  ) %>%
  kbl(caption = "Proportion of Each Study Area with Mean RPI Greater Than 1") %>%
  kable_styling(
...
RPI.Above1.Flex <- RPI.Above1.Table %>%
  select(Area, Total_Pixels, Pixels_Above_1, Percentage) %>%
  rename(
    `Study Area` = Area,
    `Total Pixels` = Total_Pixels,
  ...
  flextable() %>%
  set_caption("Proportion of Each Study Area with Mean RPI Greater Than 1") %>%
  theme_booktabs() %>%
  autofit()

# Save as Word document
save_as_docx(RPI.Above1.Flex, path = "RPI_Above1_Table.docx")
```
