...
#ggsave(
#  "RPI_Three_Panel_Figure_v1.png",
#  plot = Final.Combined.Figure.1,
#  width = 16,
#  height = 16,
#  dpi = 300,
#  bg = "white"
...
```{r}
## FILTERING OUT THE "ALL.ZONES"

# Define zones to keep
zones_bottom <- c(
  "Amboseli Park",
  "Tsavo East",
  "Tsavo West",
...
# Filter the All.Zones.Composite to remove "All Zones"
All.Zones.Composite.Bottom <- All.Zones.Composite %>%
  dplyr::filter(zone %in% zones_bottom) %>%
  # Important: make 'zone' a factor with levels in the same order as your palette
  mutate(zone = factor(zone, levels = names(area_colors_simulation)))

# Study Areas Map (Bottom Panel)
Study.Areas.Map.Bottom <- Study.Areas.Map.Composite.Coloured

# Convert tmap to ggplot object for patchwork
Study.Areas.Map.Bottom.GG <- ggplotify::as.ggplot(tmap_grob(Study.Areas.Map.Bottom))

# Filter Ridgeline data to match bottom panel zones
RPI.Bottom.Panel <- RPI.All.Simulation %>%
  dplyr::filter(Area %in% zones_bottom)

RPI.Ridgeline.Bottom.NoLegend <- ggplot(
  RPI.Bottom.Panel,
  ...
  scale_fill_manual(
    values = area_colors_simulation[zones_bottom],
    drop = TRUE
  ) +
  labs(
    x = "Mean Relative Productivity Index (RPI)",
...
# Combine bottom panels (map left, ridgeline right)
Bottom.Panel.Combined <- Study.Areas.Map.Bottom.GG | RPI.Ridgeline.Bottom.NoLegend +
  plot_layout(widths = c(2, 1)) +
  plot_annotation(
    tag_levels = "B",  # Panel tag B for bottom
    theme = theme(
      plot.tag = element_text(size = 16, face = "bold")
    )
  )
...
```{r}
# Combining the Bottom Panel created above with Mean RPI Static Map
...
#ggsave(
#  "RPI_Three_Panel_Figure_v2.png",
#  plot = Final.Combined.Figure.2,
#  width = 17,
#  height = 17,
#  dpi = 300,
#  bg = "white"
...
```{r}
# Combining the Spatial Distribution of RPI map with the Study Areas Distribution map and the Distribution of Theil-Sen Slopes ridgeline plot

Bottom.Row.2 <- 
  Study.Areas.Map.Bottom.GG | 
  Ridgeline.Plot.No.AllZones

Final.Three.Panel.Figure.1 <-
  Slope.Map.Composite.GG / 
  Bottom.Row.2 +
  plot_layout(
    heights = c(1.4, 1)   
  ) +
...
```{r}
# Balancing the Panel Widths in the figure above
...
Slope.Map.Composite <- tm_shape(Slope.Composite) +
  ...
  tm_layout(
    legend.position = c("left", "bottom"),
    frame = FALSE,
    inner.margins = c(0.08, 0.02, 0.08, 0.02) 
    # ↑ bottom, left margins enlarged
  )

Slope.Map.Composite.GG <- ggplotify::as.ggplot(tmap_grob(Slope.Map.Composite))

# Final three-panel figure: top vs bottom = 5:5
Final.Three.Panel.Figure.2 <-
  Slope.Map.Composite.GG / 
  Bottom.Row.2 +
  plot_layout(
    heights = c(1, 1) 
...
```{r}
# Saving the final three-panel figure above

# Save the combined three-panel figure
#ggsave(
#  filename = "Final_Three_Panel_Figure_2.png",  
#  plot = Final.Three.Panel.Figure.2,            
#  width = 17,                                 
#  height = 17,                                
#  dpi = 300,                                 
#  bg = "white"                                
