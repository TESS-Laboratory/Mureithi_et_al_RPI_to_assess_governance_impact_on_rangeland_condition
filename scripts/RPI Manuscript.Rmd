...
Mean.RPI.Static.Map.Image <- ggplotify::as.ggplot(png::readPNG("Mean_RPI_Static_Map.png"))

Study.Areas.Map.Composite.Image <- ggplotify::as.ggplot(png::readPNG("Study_Areas_Map_Composite_Coloured.png"))

RPI.Ridgeline.Map.Image <- ggplotify::as.ggplot(png::readPNG("RPI_Ridgeline_Bottom_NoLegend.png"))
...
# Read images
imgA <- image_read("Mean_RPI_Static_Map.png")
imgB <- image_read("Study_Areas_Map_Composite_Coloured.png")
imgC <- image_read("RPI_Ridgeline_Bottom_NoLegend.png")

# Convert to raster grobs
gA <- rasterGrob(as.raster(imgA))
gB <- rasterGrob(as.raster(imgB))
gC <- rasterGrob(as.raster(imgC))

# Wrap each raster in ggplot
ppA <- ggplot() + annotation_custom(gA, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + theme_void()
ppB <- ggplot() + annotation_custom(gB, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + theme_void()
ppC <- ggplot() + annotation_custom(gC, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) + theme_void()

# Arrange with patchwork and labels
multi_panel_patchwork <- ppA / (ppB + ppC) +
  plot_annotation(tag_levels = 'A') &
  theme(
...
# Save multipanel figure
ggsave("Multipanel_Figure_patchwork.png", plot = multi_panel_patchwork,
       width = 12, height = 10, units = "in", dpi = 300)
